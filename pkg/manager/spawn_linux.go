...
func Spawn(id, exe string, args ...string) error {
	...
	cmd, err = mutateCmdOut(cmd, id)
	if err != nil {
		return err
	}
	// run cmd as linux user
	cmd, err = mutateCmdUser(cmd, userName)
	if err != nil {
		return err
	}
...
func mutateCmdOut(cmd *exec.Cmd, id string) (*exec.Cmd, error) {
...
func mutateCmdUser(cmd *exec.Cmd, username string) (*exec.Cmd, error) {
	// Look up the user
